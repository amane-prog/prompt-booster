...
const stripe = new Stripe(STRIPE_SECRET_KEY) // apiVersionは任意（型ズレするなら外してOK）

const PORTAL_CONFIGURATION_ID =
...
// ここがポイント：Headers/ReadonlyHeaders/RequestHeadersの差異を吸収
...
export async function POST() {
    try {
        ...
        // ★ Next 15 で Promise のことがある → await で統一
        ...
        // 2) 未保存なら検索→なければ作成→DB保存
        if (!customerId) {
            let found: Stripe.Customer | null = null
            try {
                const s = await stripe.customers.search({
